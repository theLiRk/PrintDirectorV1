...
async def serve(args):
 try:
  ...
  print(f"PrintDirector configuration error: {e}",file=sys.stderr); return 2
 ...
 rt=Runtime(cfg,args.demo,args.config)
 app=create_app(rt)
 server=uvicorn.Server(uvicorn.Config(app,host=cfg.overlay.host,port=cfg.overlay.port,log_level=cfg.logging.level.lower()))
 await rt.start()
 try:
  await server.serve()
