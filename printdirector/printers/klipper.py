...
class KlipperAdapter(PrinterAdapter):
 def __init__(self,printer_id,printer_name,url): super().__init__(printer_id,printer_name); self.url=url.rstrip('/'); self._stop=None; self._data={}; self._near=False
 def _ensure_stop_event(self):
  if self._stop is None: self._stop = asyncio.Event()
  return self._stop
 async def stop(self):
  stop = self._ensure_stop_event(); stop.set()
 async def run(self):
  stop = self._ensure_stop_event(); delay = 1
  while not stop.is_set():
   try:
    ...
    try: await asyncio.wait_for(stop.wait(),delay)
    except asyncio.TimeoutError: pass
    delay=min(delay*2,30)
 async def _connect(self):
  stop = self._ensure_stop_event(); wsurl=self.url.replace('http://','ws://').replace('https://','wss://')+'/websocket'
