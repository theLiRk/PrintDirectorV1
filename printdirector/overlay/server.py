...
class Hub:
    def __init__(self, send_timeout=1.0):
        self.clients = set()
    ...
    async def _send(self, ws, data):
        try:
            await asyncio.wait_for(ws.send_json(data), timeout=self.send_timeout)
            return True
        except Exception:
            return False
...
def load_style(runtime):
    ...
    try:
        ...
        return OverlayThemeConfig.model_validate(
            {**style.model_dump(mode="json"), **data}
        )
...
def create_app(runtime):
    ...
    def all_data():
        return [
            ...
            for status in runtime.manager.statuses().values()
        ]
    ...
    async def auth_middleware(request: Request, call_next):
        ...
        if request.url.path in {"/api/health", "/api/health/ready"}:
            return await call_next(request)
        if (
            runtime.config.auth.enabled
            and request.url.path not in {"/settings", "/preview"}
        ):
    ...
    def printer(pid, request: Request):
        check_auth(runtime.config, request)
        status = runtime.manager.statuses().get(pid)
        if not status:
            raise HTTPException(404, "Unknown printer")
        return status

    @app.get("/api/director/status")
    def director_status(request: Request):
        check_auth(runtime.config, request)
    ...
    def update_settings(request: Request, payload: dict):
        if runtime.config.auth.enabled:
            check_auth(runtime.config, request)
        payload = {key: value for key, value in payload.items() if key != "printer_ids"}
        try:
        ...
        return {
            **style.model_dump(mode="json"),
            "printer_ids": list(runtime.manager.adapters.keys()),
        }

    async def _probe_bambu(printer_config: dict, gateway: str):
        adapter = BambuAdapter(
            printer_config.get("id", "connection-test"),
            printer_config.get("name", "Bambu printer"),
            gateway,
            printer_config.get("access_code"),
            printer_config.get("serial_number"),
        )
        try:
            ...
            return {
                "ok": True,
                "message": f"Bambu printer connected via MQTT at {gateway}",
            }
        except Exception as exc:
            raise HTTPException(
                502, f"Unable to connect to Bambu MQTT at {gateway}: {exc}"
            ) from exc
        finally:
            await adapter._disconnect()

    async def _probe_printer(printer_config: dict):
        printer_type = (printer_config.get("type") or "klipper").lower()
        gateway = (
            printer_config.get("moonraker_url")
            or printer_config.get("bambu_url")
            or ""
        ).strip().rstrip("/")
        if not gateway:
            raise HTTPException(400, "Printer URL is required")
        if printer_type == "bambu":
            return await _probe_bambu(printer_config, gateway)

        headers = {"Accept": "application/json"}
        candidates = [
            gateway,
            f"{gateway}/printer/info",
            f"{gateway}/api/server",
            f"{gateway}/server",
        ]
                ...
                try:
                    async with session.get(url, headers=headers) as response:
                        ...
                        if isinstance(payload, dict) and (
                            ...
                            or "server" in payload
                        ):
                            return {
                                "ok": True,
                                "message": f"Klipper printer connected at {gateway}",
                            }
    ...
    async def update_system_config(request: Request, payload: dict):
        ...
        restart_required = []
        if (new_config.overlay.host, new_config.overlay.port) != (
            runtime.config.overlay.host,
            runtime.config.overlay.port,
        ):
            restart_required.append("overlay_bind")
        if new_config.logging != runtime.config.logging:
    ...
    async def show(target, request: Request):
        ...
        if target == "overview":
            ...
            scene = runtime.config.director.idle_scene
            printer_id = None
        elif target in runtime.director.scenes:
            scene = runtime.director.scenes[target]
            printer_id = target
        else:
            raise HTTPException(404, "Unknown target")
        await runtime.director.command_scene(scene, printer_id)
        return runtime.director.public_status()
    ...
    async def stream(action, request: Request):
        ...
        if action == "start":
            ...
            await runtime.obs.stop_stream()
        else:
            raise HTTPException(400, "Use start or stop")
        return runtime.director.public_status()

    @app.websocket("/ws/printers")
    async def websocket_printers(websocket: WebSocket):
        if runtime.config.auth.enabled:
    ...
    def settings_page():
        return (BASE / "templates/settings.html").read_text(encoding="utf-8")

    @app.get("/preview", response_class=HTMLResponse)
    def preview_page():
        return (BASE / "templates/preview.html").read_text(encoding="utf-8")

    @app.get("/overlay/overview", response_class=HTMLResponse)
    def overview():
        return (BASE / "templates/overview.html").read_text(encoding="utf-8")

    @app.get("/overlay/{pid}", response_class=HTMLResponse)
    def overlay(pid):
        if pid not in runtime.manager.adapters:
            raise HTTPException(404, "Unknown printer")
        return (
            (BASE / "templates/printer.html")
            .read_text(encoding="utf-8")
            .replace("__PRINTER_ID__", pid)
        )
